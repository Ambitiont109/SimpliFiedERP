...
const PaginationButton = ({currentPageNumber, onPrevClick, onNextClick, isPrevAvailable, isNextAvailable}) => {  
  const [currentNumber, setCurrentNumber] = useState(currentPageNumber?currentPageNumber:1)
  const handleNextClick=()=>{    
    if(isNextAvailable && onNextClick) {
      setCurrentNumber(currentNumber+1)
      onNextClick()
    }
  }
  const handlePrevClick=()=>{
    if(isPrevAvailable && onPrevClick) {
      setCurrentNumber(currentNumber-1)
      onPrevClick()
    }
  }
